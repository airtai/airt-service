...
# %% auto 0
__all__ = ['get_recent_event_for_user', 'get_count_from_training_data_ch_table', 'get_user', 'process_dataframes',
...
async def process_dataframes(
    ...
    *,
    ...
    #     diff_df = recent_events_df[recent_events_df["count"] != ch_df["count"]]

    for account_id, row in recent_events_df.iterrows():
        curr_count = ch_df.loc[[account_id], "count"].values[0]  # type: ignore
        prev_count = row["count"]

        if curr_count == prev_count and curr_check_on - row["created"] > pd.Timedelta(
...
# %% ../notebooks/Training_Status_Process.ipynb 19
async def process_training_status(username: str, fast_kafka_api_app: FastKafkaAPI):
    ...
    while True:
        ...
        with get_session_with_context() as session:
            user = await asyncify(get_user)(username, session)
            recent_events_df = await asyncify(get_recent_event_for_user)(
                username, session
            )
            ch_df = await asyncify(get_count_from_training_data_ch_table)(
