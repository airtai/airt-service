...
# %% auto 0
__all__ = ['json_datetime_sec_encoder', 'ModelType', 'ModelTrainingRequest', 'EventData', 'RealtimeData', 'TrainingDataStatus',
           'TrainingModelStart', 'Tracker', 'add_process_start_training_data', 'TrainingModelStatus', 'ModelMetrics',
           'add_process_training_model_start', 'Prediction', 'add_predictions', 'create_fastkafka_application']
    ...
    *,
    ...
    async def on_model_metrics(msg: ModelMetrics) -> None:
        ...
        person_ids = get_all_person_ids_for_account_id(
            account_id=AccountId, model_id=ModelId, application_id=ApplicationId
        )

        rng = np.random.default_rng(42)

        print(
            f"Sending predictions for ({AccountId=}, {ApplicationId=}, {ModelId=}) for {len(person_ids):,d} PersonIds started."
        )
        t0 = datetime.now()
        ...
        print(
            f"Sending predictions for ({AccountId=}, {ApplicationId=}, {ModelId=}) for {len(person_ids):,d} PersonIds finished in {datetime.now()-t0}."
        )
