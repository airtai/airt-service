...
then
    ...
    airflow connections add custom_azure_batch_default --conn-type azure_batch --conn-login ${AZURE_BATCH_ACCOUNT} --conn-password ${SHARED_KEY_CREDENTIALS} --conn-extra '{"extra__azure_batch__account_url": "https://'${AZURE_BATCH_ACCOUNT}'.westeurope.batch.azure.com"}'
